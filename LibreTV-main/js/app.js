// 全局变量
const DEFAULT_SELECTED_APIS = ["dyttzy","ruyi","bfzy","tyyszy","ffzy","heimuer","zy360","iqiyi","wolong","hwba","jisu","dbzy","mozhua","mdzy","zuid","yinghua","baidu","wujin","wwzy","ikun"];
const DEFAULT_CUSTOM_APIS = [{"name":"奈飞","url":"https://www.netflixgc.com","detail":"","isAdult":false}];
let selectedAPIs = JSON.parse(localStorage.getItem('selectedAPIs') || JSON.stringify(DEFAULT_SELECTED_APIS));
let customAPIs = JSON.parse(localStorage.getItem('customAPIs') || JSON.stringify(DEFAULT_CUSTOM_APIS));
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (!localStorage.getItem('hasInitializedDefaults')) {
        ...
        if (customAPIs.length > 0) {
            customAPIs.forEach((_, i) => selectedAPIs.push('custom_' + i));
        }
                ...
                <div class="card-hover bg-[#111] rounded-lg overflow-hidden cursor-pointer transition-all hover:scale-[1.02] h-full shadow-sm hover:shadow-md" 
                    ...
                    <div class="flex h-full">
                        ...
                        <div class="relative flex-shrink-0 search-card-img-container">
                            <img src="${PROXY_URL + encodeURIComponent(item.vod_pic)}" alt="${safeName}"
